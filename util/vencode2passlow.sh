...
ffmpeg -y -i "$FILE" -pix_fmt yuv420p -c:v libx264 -preset slow -crf $CRF -r $LIMIT_FRAMERATE -pass 1 -c:a copy -f mp4 /dev/null && \
  ffmpeg -i "$FILE" -pix_fmt yuv420p -c:v libx264 -preset slow -r $LIMIT_FRAMERATE -b:v "${VBITRATE}k" -pass 2 -c:a copy -f mp4 -movflags faststart -avoid_negative_ts 1 -hide_banner -y "${2}"
